...
struct GemSourcePreset: Identifiable, Hashable {
    let id: String
    let name: String
    let url: String

    static let rubyChina = GemSourcePreset(id: "ruby-china", name: "Ruby China", url: "https://gems.ruby-china.com/")
    static let tsinghua = GemSourcePreset(id: "tsinghua", name: "清华大学", url: "https://mirrors.tuna.tsinghua.edu.cn/rubygems/")
    static let aliyun = GemSourcePreset(id: "aliyun", name: "阿里云", url: "https://mirrors.aliyun.com/rubygems/")
    static let official = GemSourcePreset(id: "official", name: "官方源", url: "https://rubygems.org/")

    static let allPresets: [GemSourcePreset] = [.rubyChina, .tsinghua, .aliyun, .official]

    static func == (lhs: GemSourcePreset, rhs: GemSourcePreset) -> Bool {
        lhs.id == rhs.id
    }
...
struct GemSourceView: View {
    @State private var selectedPreset: GemSourcePreset = .official
    ...
    private let viewModel = GemSourceViewModel()
    ...
    private var sourceSelectionList: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 6) {
                    ForEach(GemSourcePreset.allPresets) { preset in
                        GemSourceRow(
                            name: preset.name,
                            url: preset.url,
    ...
    private var customFieldsPanel: some View {
        HStack(spacing: 12) {
            ...
            TextField("https://gems.example.com/", text: $customSourceUrl)
    ...
    private func detectCurrentSource() async {
        let currentSources = await viewModel.listGemSources()
        let firstSource = currentSources.first ?? ""

        for preset in GemSourcePreset.allPresets {
            if firstSource.hasPrefix(preset.url) || preset.url.hasPrefix(firstSource) {
        ...
        if firstSource.isEmpty {
            currentSourceName = "未设置"
        } else {
            currentSourceName = firstSource
            customSourceUrl = firstSource
            isCustom = true
        }
    }
    ...
    private func applySource() async {
        ...
        let result = await viewModel.switchGemSource(to: targetUrl)
        switch result {
        case .success(let output):
            switchResultMessage = "已切换 Gem 源到 \(targetName)\n\n地址: \(targetUrl)\n\n\(output)"
...
private struct GemSourceRow: View {
    let name: String
    let url: String
    let isSelected: Bool
    let action: () -> Void
...
// MARK: - ViewModel（轻量级，只做数据获取）

@MainActor
class GemSourceViewModel: ObservableObject {
    private let service = RVMService.shared

    func listGemSources() async -> [String] {
        return await service.listGemSources()
    }

    func switchGemSource(to url: String) async -> OperationResult {
        return await service.switchGemSource(to: url)
    }
}
